...
function render(){
 ...
 for(const h of all){assets+=holdingValue(h);if(state.prices[h.key]===undefined)unknown++;else pl+=holdingValue(h)-h.cost*(h.market==='US'?state.fx:1);}
 $('#brokerSummary').innerHTML=brokerAccounts.map(account=>{const items=all.filter(h=>h.account===account),known=items.filter(h=>state.prices[h.key]!==undefined),value=items.reduce((n,h)=>n+holdingValue(h),0),gain=known.reduce((n,h)=>n+holdingValue(h)-h.cost*(h.market==='US'?state.fx:1),0);return `<button type="button" class="broker-item" data-broker="${esc(account)}" aria-label="${esc(account)}の銘柄を表示"><span>${esc(account)}・${items.length}銘柄</span><strong>評価額 ${yen(value)}</strong><span class="${gain>=0?'positive':'negative'}">含み損益 ${yen(gain)}${known.length<items.length?'（一部未算出）':''}</span></button>`;}).join('')||'<p class="empty">保有銘柄を登録すると口座別の損益を表示します。</p>';
 const hs=displayedHoldings=selectedHoldings(all),known=hs.filter(h=>state.prices[h.key]!==undefined),selectedPL=known.reduce((sum,h)=>sum+holdingValue(h)-h.cost*(h.market==='US'?state.fx:1),0);
 $('#holdingSummary').textContent=`表示中 ${hs.length}銘柄 ／ 評価額 ${yen(hs.reduce((sum,h)=>sum+holdingValue(h),0))} ／ 含み損益 ${yen(selectedPL)}${known.length<hs.length?'（価格未設定を除く・評価額は原価で仮計上）':''}`;
...
$('#txForm').onsubmit=e=>{
 ...
 try{const t=P.transaction({...d,id:d.id||crypto.randomUUID(),quantity:Number(d.quantity),price:Number(d.price),fee:Number(d.fee),priceUnit:Number(d.priceUnit||1),amount:d.amount?Number(d.amount):undefined});
...
document.addEventListener('click',e=>{
 ...
 if(b.hasAttribute('data-edit')){const t=state.transactions[Number(b.dataset.edit)],f=$('#txForm');f.reset();for(const [k,v] of Object.entries(t)){const input=field(f,k);if(input)input.value=v;}field(f,'priceUnit').disabled=t.assetType!=='FUND';$('#txForm h2').textContent='取引を編集';$('#txDialog').showModal();}
