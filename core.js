...
function normalize(x){
 ...
 const manualPrices=Array.isArray(x.manualPrices)?x.manualPrices.filter(k=>typeof k==='string'):[];
 const autoPriceAt=x.autoPriceAt&&typeof x.autoPriceAt==='object'&&!Array.isArray(x.autoPriceAt)?Object.fromEntries(Object.entries(x.autoPriceAt).filter(([k,v])=>typeof v==='string'&&!Number.isNaN(Date.parse(v)))):{};
 ...
 return {version:2,transactions,cash,prices,yenAvgCosts,manualPrices,autoPriceAt,fx:number(x.fx??150,'為替',true),fxUpdatedAt:typeof x.fxUpdatedAt==='string'?x.fxUpdatedAt:'',fxSource:['auto','manual'].includes(x.fxSource)?x.fxSource:'',lastAccount:typeof x.lastAccount==='string'?x.lastAccount:'その他',accounts,plans:Array.isArray(x.plans)?x.plans.map(p=>{if(!p||typeof p.start!=='string'||!/^\d{4}-\d{2}-\d{2}$/.test(p.start)||!Number.isFinite(Date.parse(p.start)))fail('積立開始日が不正です');return {id:text(p.id,'積立ID'),account:text(p.account,'積立口座'),symbol:text(p.symbol,'投信コード'),name:text(p.name,'投信名'),amount:number(p.amount,'積立金額',true),start:p.start,end:typeof p.end==='string'?p.end:'',taxBucket:p.taxBucket||'tsumitate',skipDates:Array.isArray(p.skipDates)?p.skipDates.filter(d=>typeof d==='string'):[],orderedDates:Array.isArray(p.orderedDates)?p.orderedDates.filter(d=>typeof d==='string'):[]};}):[],priceDates:x.priceDates&&typeof x.priceDates==='object'&&!Array.isArray(x.priceDates)?Object.fromEntries(Object.entries(x.priceDates).filter(([k,v])=>typeof v==='string')):{},updatedAt:typeof x.updatedAt==='string'?x.updatedAt:''};
}
